...
client.on("ready", () => {
    client.user.setActivity(`D İ E G O`);
...
client.on("message", async message => {
  ...
  if(message.content === "d!role") {
   ...
   await roleData.findOne({rolid: role.id}, async(err,res) => {
      ...
      message.channel.send("Rol Yedeklendi <:onayl:903692202742513695>\n```Rol: "+role.name+"\nÜye Sayısı: "+role.members.size+"```")
      } else if(res) {
  ...
  message.guild.roles.cache.map(async(role) => {
    ...
    await channelData.findOne({ roleid: role.id }, async (err, res) => {
      if (!res) {
        ...
        message.channel.send("Rol Kanal İzinleri Yedeklendi <:onayl:903692202742513695>\n```Rol: "+role.name+"\nKanal sayısı: "+channels+"```", {split: true})
      } else {
        res.roleid = role.id;
...
client.on("roleDelete", role => {
  client.channels.cache.get("781661216077512714").send("<:onaysz:903692188922286090> Silinen Rol ID: `" + role.id + "`");
  client.channels.cache.get("781661216077512714").send("<:onaysz:903692188922286090> Silinen Rol İsim: `" + role.name + "`");
});
